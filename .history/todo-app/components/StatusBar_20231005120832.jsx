  ...
  return (
    <Box fontWeight={"700"} color={"grey"}>
        {isMobileView && <Flex direction={"column"}>
            ...
            <Box h="3em" minW={"100%"} backgroundColor={"#242424"}>
                <Flex gap h={"3em"} justifyContent={"center"} alignItems={"center"} borderRadius={"10px"} backgroundColor={colorMode === "light" ? "white" : "#1a202c"}>
                <Text onClick={handleClearAllClick} cursor={"pointer"} _hover={{color: colorMode === "light" ? "black" : "white"}}>
                    All
                </Text>
                <Text onClick={handleClearAllClick} cursor={"pointer"} _hover={{color: colorMode === "light" ? "black" : "white"}}>
